function Implicit_Poisson_Solver
    ...
    fig_save_name = 'New_Final_WiSE_21m_fp4_sticky_cl_dist_m_';%Figure Save name
    ...
    %[~,cI] = min(abs(7.5-xint))
    figure()
    temp=log10(clms_a{3});%log10(clms_a{cI});
    temp(temp<-8)=nan;%8%Need to choose our cutoff magnitude
    fig=pcolor(-0.5:size(temp,1)-1.5,-0.5:size(temp,2)-1.5,temp')
    set(fig, 'EdgeColor', 'none')
    clim([-8,0])
...
function [clms,cubeAggLs] = WiSE_sti_theory_func_count(fa,fc,xic,xia,Lbar,Xp,Yp,zp,EP)%[rho,ip_vf,agg_vf,sum_cl,sum_cm,sum_cs] = WiSE_sti_theory_func_count(fa,fc,Lbar,Xp,Yp,zp,EP)
...
function SN = sti_stockmeyer_prob_4sum(l,m,XX,YY,fc,fa)%added sti_
