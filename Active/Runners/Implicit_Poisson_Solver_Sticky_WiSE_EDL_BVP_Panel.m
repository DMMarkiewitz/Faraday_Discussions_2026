...
fig_save_name = 'WiSE_21m_fp4_sticky_m_2_01';%Figure Save name
...
elseif mm == 21
    ...
    v0 = (2.1744e-29+2.2863e-29+2.2456e-29)/3; %m^3 
...
ft = tiledlayout(4,2,'TileSpacing','tight','padding','compact');
letters = 18;
...
nexttile
hold on
plot(xint,phi(1,:),'r-','linewidth',1.5)
hold on
plot(xint,phi(2,:),'b--','linewidth',1.5)
plot(xint,phi(3,:),'k:','linewidth',1.5)
xlim([0,30])
ylim([-0.1,1.29])
box on
...
ylabel(' ')
text(.6,1.15,'\bf{e)}','interpreter','latex','FontSize',letters)
...
nexttile
hold on
plot(xint,(1+fc/xic)*phi(1,:).*(prob(3,:)).^fc,'r-','linewidth',1.5)
plot(xint,phi(2,:).*(1-prob(2,:)).^fa,'b--','linewidth',1.5)
plot(xint,phi(3,:).*(1-prob(4,:)),'k:','linewidth',1.5)
plot(xint,1-phi(3,:).*(1-prob(4,:))-phi(2,:).*(1-prob(2,:)).^fa-(1+fc/xic)*phi(1,:).*(prob(3,:)).^fc,'m-.','linewidth',1.5)
xlim([0,30])
ylim([-0.1,1.29])
box on
leg=legend({'$\bar{\phi}_{104}$','$\bar{\phi}_{010}$','$\bar{\phi}_{001}$','$\bar{\phi}_{Agg}$'},'location','north','interpreter','latex','Orientation','horizontal','box','off');
leg.ItemTokenSize = [18,18];
text(.6,1.15,'\bf{f)}','interpreter','latex','FontSize',letters)
set(groot, 'defaultAxesTickLabelInterpreter','latex'); 
set(groot, 'DefaultTextInterpreter', 'latex')
ax = gca;
ax.FontSize = 16;
...
nexttile
hold on
plot(xint,prob(1,:),'r-','linewidth',1.5)
plot(xint,prob(3,:),'r--','linewidth',1.5)
plot(xint,prob(2,:),'b:','linewidth',1.5)
plot(xint,prob(4,:),'k-.','linewidth',1.5)
xlim([0,30])
ylim([-0.1,1.39])
box on
leg=legend({'$\bar{p}_{+-}$','$\bar{p}_{+0}$','$\bar{p}_{-+}$','$\bar{p}_{0+}$'},'location','north','interpreter','latex','Orientation','horizontal','box','off');
leg.ItemTokenSize = [18,18];
text(0.6,1.25,'\bf{g)}','interpreter','latex','FontSize',letters)
set(groot, 'defaultAxesTickLabelInterpreter','latex'); 
set(groot, 'DefaultTextInterpreter', 'latex')
ax = gca;
ax.FontSize = 16;
ax.XTick = 0:5:30;
ax.YTick = 0:.2:1.2;%1.2
set(gca,'YTickLabel',[]);
set(gca,'XTickLabel',[]);
g=gcf;
g.Renderer='painters';
...
nexttile
hold on
plot(xint,1/((fc-1)*(fa-1))*ones(1,length(xint)),'k--','linewidth',1.5)
plot(xint,ppmpmp,'r-','linewidth',1.5)
xlim([0,30])        
ylim([-0.05,.675])
box on
leg=legend('$((f_+-1)(f_--1))^{-1}$','location','north','interpreter','latex','Orientation','horizontal','box','off');
leg.ItemTokenSize = [18,18];
text(0.6,0.6,'\bf{h)}','interpreter','latex','FontSize',letters)
...
exportgraphics(ft,[fig_save_name,'.eps'],'Resolution',600)
exportgraphics(ft,[fig_save_name,'.jpeg'],'Resolution',600)
saveas(ft,[fig_save_name,'.fig'])
...
function [xint, Phi, DPhi, phi, prob, ppmpmp, epsr, rho] = MPB(xGuess,yGuess,cutoff,qs,p,fc,fa,xic,xia,phi_bulk,f_I_rho,f_I_phi_c,f_I_phi_a,f_I_phi_0,f_I_pc,f_I_pa,f_I_pp0,f_I_p0,f_I_epsr,f_I_depsrdu,f_I_depsrddu,options)
    ...
    for i = 1:length(Sxint(2,:))
        %So one can back calc qs with units of C/m^2
